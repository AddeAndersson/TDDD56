...
// Compiled only if LOADBALANCE = 1
	...
	if(args->id == 0 || args->id == 1)
	{
		// Define the region compute_chunk() has to compute
		// Entire height: from 0 to picture's height
		parameters->begin_h = args->id * (parameters->height / 2);
		parameters->end_h = (args->id + 1) * (parameters->height / 2);
		// Entire width: from 0 to picture's width
		parameters->begin_w = 0;
	...
	else if(args->id == 3 || args->id == 4)
	{
		parameters->begin_h = (args->id - 2) * (parameters->height / 4);
		parameters->end_h = (args->id - 1) * (parameters->height / 4);

		parameters->begin_w = parameters->width / 2;
		parameters->end_w = 3 * (parameters->width / 4);
	}
	else if(args->id == 6 || args->id == 7)
	{
		parameters->begin_h = (args->id - 5) * (parameters->height / 4);
		parameters->end_h = (args->id - 4) * (parameters->height / 4);
