...
class CLWrapperClass_average_precompiled_OverlapKernel_gaussian_kernel_1d_col
{
public:
...
{ return v.data[i]; }
static unsigned char gaussian_kernel_1d_col(skepu_region1d_unsigned__space__char m, skepu_vec_proxy_float stencil)
{
        //float scaling = 1.0 / (m.oi * 2 + 1);
        float res = 0;
	unsigned int idx = 0;
        for(int i = -m.oi; i <= m.oi; ++i) {
                res += skepu_region_access_1d_unsigned__space__char(m,i) * stencil.data[idx];
		idx++;
	}
...
__kernel void average_precompiled_OverlapKernel_gaussian_kernel_1d_col_Vector( __global unsigned char* skepu_output, __global unsigned char* m, __global float *skepu_container_stencil, size_t skepu_size_stencil, 
...
skepu_region1d_unsigned__space__char skepu_region = { .data = &sdata[tid+skepu_overlap], .oi = skepu_overlap, .stride = 1 };
	...
	if (skepu_i >= out_offset && skepu_i < out_offset + out_numelements)
		skepu_output[skepu_i - out_offset] = gaussian_kernel_1d_col(skepu_region, stencil);
}

__kernel void average_precompiled_OverlapKernel_gaussian_kernel_1d_col_MatRowWise( __global unsigned char* skepu_output, __global unsigned char* m, __global float *skepu_container_stencil, size_t skepu_size_stencil, 
...
__kernel void average_precompiled_OverlapKernel_gaussian_kernel_1d_col_MatColWise( __global unsigned char* skepu_output, __global unsigned char* m, __global float *skepu_container_stencil, size_t skepu_size_stencil, 
	__global unsigned char* skepu_wrap, size_t skepu_n, size_t skepu_overlap, size_t out_offset, size_t out_numelements,
...
skepu_region1d_unsigned__space__char skepu_region = { .data = &sdata[tid+skepu_overlap], .oi = skepu_overlap, .stride = 1 };
	...
	if ((arrInd >= out_offset) && (arrInd < out_offset+out_numelements))
		skepu_output[arrInd-out_offset] = gaussian_kernel_1d_col(skepu_region, stencil);
}

__kernel void average_precompiled_OverlapKernel_gaussian_kernel_1d_col_MatColWiseMulti( __global unsigned char* skepu_output, __global unsigned char* m, __global float *skepu_container_stencil, size_t skepu_size_stencil, 
...
skepu_region1d_unsigned__space__char skepu_region = { .data = &sdata[tid+skepu_overlap], .oi = skepu_overlap, .stride = 1 };
	...
	if (arrInd < out_numelements )
		skepu_output[arrInd] = gaussian_kernel_1d_col(skepu_region, stencil);
}
)###");
		...
		for (skepu::backend::Device_CL *device : skepu::backend::Environment<int>::getInstance()->m_devices_CL)
		{
			...
			cl_kernel kernel_vector = clCreateKernel(program, "average_precompiled_OverlapKernel_gaussian_kernel_1d_col_Vector", &err);
			CL_CHECK_ERROR(err, "Error creating MapOverlap 1D vector kernel 'average_precompiled_OverlapKernel_gaussian_kernel_1d_col'");

			cl_kernel kernel_matrix_row = clCreateKernel(program, "average_precompiled_OverlapKernel_gaussian_kernel_1d_col_MatRowWise", &err);
			CL_CHECK_ERROR(err, "Error creating MapOverlap 1D matrix row-wise kernel 'average_precompiled_OverlapKernel_gaussian_kernel_1d_col'");

			cl_kernel kernel_matrix_col = clCreateKernel(program, "average_precompiled_OverlapKernel_gaussian_kernel_1d_col_MatColWise", &err);
			CL_CHECK_ERROR(err, "Error creating MapOverlap 1D matrix col-wise kernel 'average_precompiled_OverlapKernel_gaussian_kernel_1d_col'");

			cl_kernel kernel_matrix_col_multi = clCreateKernel(program, "average_precompiled_OverlapKernel_gaussian_kernel_1d_col_MatColWiseMulti", &err);
			CL_CHECK_ERROR(err, "Error creating MapOverlap 1D matrix col-wise multi kernel 'average_precompiled_OverlapKernel_gaussian_kernel_1d_col'");
	...
	(
		size_t deviceID, size_t localSize, size_t globalSize,
		 skepu::backend::DeviceMemPointer_CL<unsigned char> *skepu_output, skepu::backend::DeviceMemPointer_CL<unsigned char> *skepu_input, std::tuple<skepu::Vector<float> *, skepu::backend::DeviceMemPointer_CL<float> *> skepu_container_stencil, 
	...
	)
	{
		...
		skepu::backend::cl_helpers::setKernelArgs(kernel,  skepu_output->getDeviceDataPointer(), skepu_input->getDeviceDataPointer(), std::get<1>(skepu_container_stencil)->getDeviceDataPointer(), std::get<0>(skepu_container_stencil)->size(), 
			skepu_wrap->getDeviceDataPointer(), skepu_n, skepu_overlap, out_offset, out_numelements, poly, pad);
		clSetKernelArg(kernel, 4 + 7, sharedMemSize, NULL);
	...
	)
	{
		...
		clSetKernelArg(kernel, 4 + 9, sharedMemSize, NULL);
	...
	)
	{
		...
		clSetKernelArg(kernel, 4 + 10, sharedMemSize, NULL);
		cl_int err = clEnqueueNDRangeKernel(skepu::backend::Environment<int>::getInstance()->m_devices_CL.at(deviceID)->getQueue(),
			kernel, 1, NULL, &globalSize, &localSize, 0, NULL, NULL);
	...
	)
	{
		...
		clSetKernelArg(kernel, 4 + 11, sharedMemSize, NULL);
		cl_int err = clEnqueueNDRangeKernel(skepu::backend::Environment<int>::getInstance()->m_devices_CL.at(deviceID)->getQueue(),
			kernel, 1, NULL, &globalSize, &localSize, 0, NULL, NULL);
