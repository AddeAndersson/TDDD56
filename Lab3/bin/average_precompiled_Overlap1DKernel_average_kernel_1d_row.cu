...
__global__ void average_precompiled_Overlap1DKernel_average_kernel_1d_row_MapOverlapKernel_CU(
...
)
{
   ...
   while(i<(n+overlap-1))
   {
      ...
      if( (i >= out_offset) && (i < out_offset+out_numelements) )
      	 output[i-out_offset] = skepu_userfunction_skepu_skel_1conv_row_average_kernel_1d_row::CU({(int)overlap, 1, &sdata[tid + overlap]} , elemPerPx);
...
__global__ void average_precompiled_Overlap1DKernel_average_kernel_1d_row_MapOverlapKernel_CU_Matrix_Row(
	unsigned char* input, unsigned long elemPerPx,  unsigned char* output,
...
__global__ void average_precompiled_Overlap1DKernel_average_kernel_1d_row_MapOverlapKernel_CU_Matrix_Col(
	unsigned char* input, unsigned long elemPerPx,  unsigned char* output,
	unsigned char* wrap, size_t n, size_t out_offset, size_t out_numelements,
...
)
{
   ...
   {
      output[arrInd-out_offset] = skepu_userfunction_skepu_skel_1conv_row_average_kernel_1d_row::CU({(int)overlap, 1, &sdata[tid + overlap]} , elemPerPx);
   }
}

__global__ void average_precompiled_Overlap1DKernel_average_kernel_1d_row_MapOverlapKernel_CU_Matrix_ColMulti(
	unsigned char* input, unsigned long elemPerPx,  unsigned char* output,
...
)
{
   ...
   {
      output[arrInd] = skepu_userfunction_skepu_skel_1conv_row_average_kernel_1d_row::CU({(int)overlap, 1, &sdata[tid + overlap]} , elemPerPx);
   }
}
