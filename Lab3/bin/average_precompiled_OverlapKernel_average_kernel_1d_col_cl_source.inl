...
class CLWrapperClass_average_precompiled_OverlapKernel_average_kernel_1d_col
{
public:
...
static unsigned char average_kernel_1d_col(skepu_region1d_unsigned__space__char m)
{
	float scaling = 1.0 / (m.oi * 2 + 1);
	float res = 0;

	for(int i = -m.oi; i <= m.oi; ++i)
		res += skepu_region_access_1d_unsigned__space__char(m,i);

	return res * scaling;
}


__kernel void average_precompiled_OverlapKernel_average_kernel_1d_col_Vector( __global unsigned char* skepu_output, __global unsigned char* m, 
...
)
{
	...
	if (skepu_i >= out_offset && skepu_i < out_offset + out_numelements)
		skepu_output[skepu_i - out_offset] = average_kernel_1d_col(skepu_region);
}

__kernel void average_precompiled_OverlapKernel_average_kernel_1d_col_MatRowWise( __global unsigned char* skepu_output, __global unsigned char* m, 
...
__kernel void average_precompiled_OverlapKernel_average_kernel_1d_col_MatColWise( __global unsigned char* skepu_output, __global unsigned char* m, 
	__global unsigned char* skepu_wrap, size_t skepu_n, size_t skepu_overlap, size_t out_offset, size_t out_numelements,
	...
	)
{
	...
	if ((arrInd >= out_offset) && (arrInd < out_offset+out_numelements))
		skepu_output[arrInd-out_offset] = average_kernel_1d_col(skepu_region);
}

__kernel void average_precompiled_OverlapKernel_average_kernel_1d_col_MatColWiseMulti( __global unsigned char* skepu_output, __global unsigned char* m, 
...
)
{
	...
	if (arrInd < out_numelements )
		skepu_output[arrInd] = average_kernel_1d_col(skepu_region);
}
)###");
		...
		for (skepu::backend::Device_CL *device : skepu::backend::Environment<int>::getInstance()->m_devices_CL)
		{
			...
			cl_kernel kernel_vector = clCreateKernel(program, "average_precompiled_OverlapKernel_average_kernel_1d_col_Vector", &err);
			CL_CHECK_ERROR(err, "Error creating MapOverlap 1D vector kernel 'average_precompiled_OverlapKernel_average_kernel_1d_col'");

			cl_kernel kernel_matrix_row = clCreateKernel(program, "average_precompiled_OverlapKernel_average_kernel_1d_col_MatRowWise", &err);
			CL_CHECK_ERROR(err, "Error creating MapOverlap 1D matrix row-wise kernel 'average_precompiled_OverlapKernel_average_kernel_1d_col'");

			cl_kernel kernel_matrix_col = clCreateKernel(program, "average_precompiled_OverlapKernel_average_kernel_1d_col_MatColWise", &err);
			CL_CHECK_ERROR(err, "Error creating MapOverlap 1D matrix col-wise kernel 'average_precompiled_OverlapKernel_average_kernel_1d_col'");

			cl_kernel kernel_matrix_col_multi = clCreateKernel(program, "average_precompiled_OverlapKernel_average_kernel_1d_col_MatColWiseMulti", &err);
			CL_CHECK_ERROR(err, "Error creating MapOverlap 1D matrix col-wise multi kernel 'average_precompiled_OverlapKernel_average_kernel_1d_col'");
	...
	(
		size_t deviceID, size_t localSize, size_t globalSize,
		 skepu::backend::DeviceMemPointer_CL<unsigned char> *skepu_output, skepu::backend::DeviceMemPointer_CL<unsigned char> *skepu_input, 
	...
	)
	{
		...
		skepu::backend::cl_helpers::setKernelArgs(kernel,  skepu_output->getDeviceDataPointer(), skepu_input->getDeviceDataPointer(), 
			skepu_wrap->getDeviceDataPointer(), skepu_n, skepu_overlap, out_offset, out_numelements, poly, pad);
		clSetKernelArg(kernel, 2 + 7, sharedMemSize, NULL);
	...
	)
	{
		...
		clSetKernelArg(kernel, 2 + 9, sharedMemSize, NULL);
	...
	)
	{
		...
		clSetKernelArg(kernel, 2 + 10, sharedMemSize, NULL);
		cl_int err = clEnqueueNDRangeKernel(skepu::backend::Environment<int>::getInstance()->m_devices_CL.at(deviceID)->getQueue(),
			kernel, 1, NULL, &globalSize, &localSize, 0, NULL, NULL);
	...
	)
	{
		...
		clSetKernelArg(kernel, 2 + 11, sharedMemSize, NULL);
		cl_int err = clEnqueueNDRangeKernel(skepu::backend::Environment<int>::getInstance()->m_devices_CL.at(deviceID)->getQueue(),
			kernel, 1, NULL, &globalSize, &localSize, 0, NULL, NULL);
