...
typedef struct {
	...
	size_t size;
} skepu_vec_proxy_unsigned__space__char;
unsigned char skepu_vec_proxy_access_unsigned__space__char(skepu_vec_proxy_unsigned__space__char v, size_t i)
{ return v.data[i]; }
static unsigned char median_kernel(skepu_region2d_unsigned__space__char image, skepu_vec_proxy_unsigned__space__char sortedasd, unsigned long elemPerPx)
{
	// your code here
	unsigned int idx = 0;
	...
	for(int y = -image.oi; y <= image.oi; y++) {
		for(int x = -image.oj; x <= image.oj; x += elemPerPx) {
			sorted[idx] = skepu_region_access_2d_unsigned__space__char(image,y,x);
			idx++;
		}
	}
	float temp = 0;
...
__kernel void median_precompiled_Overlap2DKernel_median_kernel( __global unsigned char* skepu_output, __global unsigned char* image, __global unsigned char *skepu_container_sortedasd, size_t skepu_size_sortedasd, unsigned long elemPerPx, 
	...
	__local unsigned char* sdata)
{
	...
	skepu_vec_proxy_unsigned__space__char sortedasd = { .data = skepu_container_sortedasd, .size = skepu_size_sortedasd };
skepu_region2d_unsigned__space__char skepu_region = { .data = &sdata[(get_local_id(1) + skepu_overlap_y) * sharedCols + (get_local_id(0) + skepu_overlap_x)], .oi = skepu_overlap_y, .oj = skepu_overlap_x, .stride = sharedCols };
	...
	{
#if !0
		skepu_output[y * out_cols + x] = median_kernel(skepu_region, sortedasd, elemPerPx);
#else
		skepu_multiple skepu_out_temp = median_kernel(skepu_region, sortedasd, elemPerPx);
	...
	(
		size_t deviceID, size_t localSize[2], size_t globalSize[2],
		 skepu::backend::DeviceMemPointer_CL<unsigned char> *skepu_output, skepu::backend::DeviceMemPointer_CL<unsigned char> *skepu_input, std::tuple<skepu::Vector<unsigned char> *, skepu::backend::DeviceMemPointer_CL<unsigned char> *> skepu_container_sortedasd, unsigned long elemPerPx, 
		size_t out_rows, size_t out_cols, size_t skepu_overlap_y, size_t skepu_overlap_x,
		size_t in_pitch, size_t sharedRows, size_t sharedCols,
		size_t sharedMemSize
	)
	{
		skepu::backend::cl_helpers::setKernelArgs(kernels(deviceID),  skepu_output->getDeviceDataPointer(), skepu_input->getDeviceDataPointer(), std::get<1>(skepu_container_sortedasd)->getDeviceDataPointer(), std::get<0>(skepu_container_sortedasd)->size(), elemPerPx, 
			out_rows, out_cols, skepu_overlap_y, skepu_overlap_x, in_pitch, sharedRows, sharedCols);
		clSetKernelArg(kernels(deviceID), 5 + 7, sharedMemSize, NULL);
