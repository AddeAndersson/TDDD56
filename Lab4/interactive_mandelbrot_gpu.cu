...
// Image data
    ...
    unsigned char *pixels_gpu = NULL;
    __managed__ int gImageWidth, gImageHeight;
    __constant__ const int blocksize = 16;
    __constant__ const int size = DIM*DIM*4;

// Init image data
void initBitmap(int width, int height)
{
    if (pixels) free(pixels);
    if (pixels_gpu) cudaFree(pixels_gpu);
    cudaMalloc((void**)&pixels_gpu, size);
...
{    
    ...
    ptr[offset*4 + 0] = red; // R
    ptr[offset*4 + 1] = green; // G
    ptr[offset*4 + 2] = blue; // B
...
void Draw()
{
    ...
    cudaMemcpy(pixels_gpu, pixels, size, cudaMemcpyHostToDevice);
    dim3 numOfThreads( blocksize, blocksize);
    dim3 numOfBlocks( gImageWidth / numOfThreads.x, gImageHeight / numOfThreads.y );
    computeFractal<<<numOfBlocks, numOfThreads>>>(pixels_gpu);
    cudaThreadSynchronize();
    cudaMemcpy(pixels, pixels_gpu, size, cudaMemcpyDeviceToHost);
